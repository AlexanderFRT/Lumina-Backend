...
# Copia la aplicación de backend al contenedor
COPY . .
...
RUN dotnet build "./Lumina-Backend.csproj" -c Release -o /app/build

# Etapa de publicación
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./Lumina-Backend.csproj" -c Release -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "Lumina_Backend.dll"]